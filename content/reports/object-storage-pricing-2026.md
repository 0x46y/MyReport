...
date: "2026-05-12"
excerpt: "画像、音声、動画、AI生成物を預けるときに、保存料金だけでなく外向き転送料（egress）と操作料金まで含めて選び方を整理する。"
category: "Cloud"
...
SKU、無料枠、従量課金、egress、S3互換などの用語は [開発・クラウド・CADの記事で使う用語メモ](/notes/system-development-terms) に短く整理している。
...
無料枠を見るときは、少なくとも「保存できる容量」「読み書きの操作回数」「外向き転送量」「最低保存期間や最低課金」を分けて確認したい。同じ無料枠でも、保存容量に余裕があるサービスと、読み取り回数や転送量で制限に近づくサービスでは、向いている用途が変わる。
...
ストレージ単体ではなく、クラウド全体のランニングコストとして見る観点は [クラウド選定で見落としやすいコストと依存関係](/reports/cloud-running-cost-vendor-lockin) にも整理している。

## 一番見落としやすいのは外向き転送（egress）

オブジェクトストレージを初めて使うときに見落としやすいのは、外向き転送（egress）の料金だ。保存料金は安く見えても、ユーザーが大量にダウンロードすると転送料金が一気に増える。
