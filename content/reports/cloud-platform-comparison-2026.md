...
クラウドの月額費用や依存関係については [クラウド選定で見落としやすいコストと依存関係](/reports/cloud-running-cost-vendor-lockin) にも整理している。
...
ローカルで動いたものが本番環境でも同じように動くとは限らない。特にEdge環境、Webhook、署名検証、時刻、ヘッダー、raw bodyの扱いが関係する処理は早めに本番相当の環境で確認したい。この点は [ローカルで動いたのに本番で動かないのはなぜか](/reports/local-production-edge-runtime-differences) に別途整理する。
